...
from utils_.data_helpers import LoadSQuADQuestionAnsweringDataset
from utils_.log_helper import logger_init
...
class ModelConfig:
    def __init__(self):
        ...
        self.train_file_path = os.path.join(self.dataset_dir, 'train_medicine_data.json')
        self.test_file_path = os.path.join(self.dataset_dir, 'test_medicine_data.json')
        ...
        self.max_answer_len = 100  # 在对候选进行筛选时，对答案最大长度的限制
        self.is_sample_shuffle = True  # 是否对训练集进行打乱
        self.use_torch_multi_head = False  # 是否使用PyTorch中的multihead实现
        self.batch_size = 2
        self.max_sen_len = 500  # 最大句子长度，即 [cls] + question ids + [sep] +  context ids + [sep] 的长度
...
def train(config):
    ...
    if os.path.exists(config.model_save_path):
        ...
        model.load_state_dict(loaded_paras['model'])
        loaded_optimizer_state=loaded_paras['optimizer']
        optimizer.load_state_dict(loaded_optimizer_state)
        epoch=loaded_paras['epoch']
